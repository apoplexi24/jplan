...
def create_cron_job(
    ...
    python_cmd = (
        ...
        f"{parameters or {}}, \"{kernel_name or ''}\")'"
    )

    # Create the cron job command with absolute paths
    cron_cmd = f"{schedule} cd {os.getcwd()} && {python_cmd} >> {output_path.parent}/cron.log 2>&1"
    ...
    try:
        ...
        print(f"Logs will be written to: {output_path.parent}/cron.log")
